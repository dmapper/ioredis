  ...
  function connectToNext() {
    ...
    _this.resolve(endpoint, function (err, resolved) {
      ...
      if (resolved) {
        ...
        _this.stunnel(resolved).then(function(resolved) {
          debug('resolved: %s:%s', resolved.host, resolved.port);
          if (_this.options.tls) {
            _.assign(resolved, _this.options.tls);
            _this.stream = tls.connect(resolved);
          } else {
...
SentinelConnector.prototype.stunnel = async function (endpoint) {

  if (!this.options.tls || !this.options.sslPort) return endpoint

  const stunnels = []
  for (const sentinel of this.options.sentinels ) {
    const ip = await new Promise((resolve, reject) => {
      dns.lookup(sentinel.host, (err, ip) => {
        if (err) return reject(err)
        resolve(ip)
      })
    })
    if (ip === sentinel.host) continue

    stunnels.push({
      redisHost: ip,
      redisPort: '6379',
      stunnelHost: sentinel.host,
      stunnelPort: this.options.sslPort
    })

  }

  var resolved;
  stunnels.forEach(function(stunnel) {
